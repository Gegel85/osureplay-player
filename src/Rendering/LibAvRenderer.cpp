...
namespace OsuReplayPlayer
{
		...
		_size(size)
	{
		...
		this->_initCodec(size, fps, bitRate);
		this->_initFrame();
	...
	LibAvRenderer::~LibAvRenderer()
	{
		...
		if (this->_stream) {
			...
			fwrite(endcode, 1, sizeof(endcode), this->_stream);
	...
	void LibAvRenderer::_prepareFrame()
	{
		...
		/* prepare the frame */
		/* Y */
		for (int y = 0; y < this->_codecContext->height; y++) {
			for (int x = 0; x < this->_codecContext->width; x++) {
		...
		/* Cb Cr */
		for (int y = 0; y < this->_codecContext->height / 2; y++) {
			for (int x = 0; x < this->_codecContext->width / 2; x++) {
	...
	void LibAvRenderer::_initCodec(sf::Vector2u size, unsigned fps, size_t bitRate)
	{
		const AVCodec *codec;
	...
	void LibAvRenderer::_initFrame()
	{
		...
		frame->format = this->_codecContext->pix_fmt;
	...
	void LibAvRenderer::_flush(bool sendFrame)
	{
		...
		int ret = avcodec_send_frame(this->_codecContext, sendFrame ? this->_frame : nullptr);
		if (ret < 0)
