...
OsuIntegerVector	bezier(OsuIntegerVectorArray points, double percent)
{
	...
	if (!pts.content)
		display_error("Memory allocation error (%luB)\n", (unsigned long)(pts.length * sizeof(*pts.content)));
...
OsuIntegerVectorArray	getBezierPoints(OsuIntegerVectorArray points, OsuIntegerVector pos)
{
	...
	if (!pts)
		display_error("Memory allocation error (%luB)\n", (unsigned long)sizeof(*pts));
	pts->length = 1;
	pts->content = malloc(sizeof(*pts->content));
	if (!pts->content)
		display_error("Memory allocation error (%luB)\n", (unsigned long)sizeof(*pts->content));
	*pts->content = pos;
	for (unsigned i = 0; i < points.length; i++) {
		...
		if (!pts->content)
			display_error("Memory allocation error (%luB)\n", (unsigned long)((pts[arrLen - 1].length + 1) * sizeof(*pts[arrLen - 1].content)));
	...
	if (!result.content)
		display_error("Memory allocation error (%luB)\n", (unsigned long)(result.length * sizeof(*result.content)));
	for (unsigned i = 0; i < arrLen; i++)
